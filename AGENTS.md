...
## 21.1. Wymagane narzędzia AI (Laravel Boost + Statamic Boost)

Projekt używa dwóch pakietów MCP rozszerzających możliwości agentów AI:

- `laravel/boost` — MCP server z narzędziami Laravel (schema bazy, logi, trasy, konfiguracja, Tinker)
- `chrisvasey/statamic-boost` — rozszerzenie o narzędzia Statamic (kolekcje, blueprinty, taksonomie, asset containery)

### Pakiety Composer (automatyczne)

Oba pakiety są w `composer.json` jako `require-dev`. Po `git pull` i `composer install` są dostępne automatycznie — nie wymaga dodatkowych kroków.

### Plugin Claude Code (per-machine — wymaga ręcznej instalacji)

Plugin `laravel-boost` w Claude Code jest instalowany **per-machine** i nie jest synchronizowany przez Git. Na każdym nowym komputerze lub świeżej instalacji Claude Code należy:

**Krok 1** — zainstalować plugin w Claude Code (jednorazowo):
```
/plugin   →  wyszukać "laravel-boost"  →  zainstalować
/reload-plugins
```

**Krok 2** — skonfigurować MCP server w katalogu projektu (jednorazowo po `composer install`):
```bash
php artisan boost:install
```

Komenda `boost:install` zapisuje konfigurację MCP do pliku `.claude/mcp.json` projektu oraz guidelines do `CLAUDE.md`. Jeśli przy jej uruchomieniu pojawi się błąd renderowania Blade (ParseError z kodem Antlers) — można go zignorować, guidelines Statamic są już wpisane ręcznie w `CLAUDE.md`.

### Weryfikacja działania

```bash
php artisan boost:mcp   # powinien uruchomić MCP server bez błędów
```
