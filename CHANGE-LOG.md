...
- **8 hardcoded `<a href="/services">`** w `resources/views/page_builder/service_section.antlers.html` (wszystkie 7 wariantów `show_type`: home-page-one/two/three/four, service-page-one/two/three) → `{{ button_entry }}<a href="{{ url }}">{{ /button_entry }}`. Powodowały 404 dla PL gdzie strona kolekcji services to `/oferta` (rename z 2026-06-20 `FEATURE-services-route-pl-oferta`).
- Każdy button teraz w warunku `{{ if button_text && button_entry }}` — jeśli user nie wybierze strony w picker, button nie wyświetli się (zero 404, lepsze UX niż broken link).

### Dodano

- **`resources/fieldsets/service_section.yaml`** — pole `button_entry` (`type: entries`, `collections: [pages]`, `max_items: 1`, display "Strona docelowa buttona") + przemianowane display istniejącego `collections` field na "Kolekcja źródłowa (skąd brać wpisy do wyświetlenia)" dla jasności (rozróżnienie od button URL).

### Decyzje techniczne

- **Wzorzec analogiczny do `services_grid_section.section_button_entry`** z 2026-06-20 (`FEATURE-section-button-entry-picker`) — entries picker na pages collection, Statamic native `{{ url }}` w entry loop automatycznie generuje URL per locale (PL: `/oferta`, EN: `/services`, inne locale: locale-specific gdy strona zostanie utworzona).
- **Bez fallback do hardcoded URL** — jeśli `button_entry` pusty, button nie wyświetla się. Bezpieczniej niż graceful fallback (który mógł by zwrócić 404 dla nowych locale).
- **Migration:** istniejące wpisy używające service_section nie mają `button_entry` w danych → buttony znikną do czasu gdy user wybierze stronę w picker w CP. To akceptowalne (user widzi że trzeba uzupełnić, zamiast 404).

### Wdrożono na dev.skalisty.pl

- Rsync 2 plików (8 KB): fieldset + widok
- Post-deploy: view:clear + statamic:stache:refresh — OK
- Walidacja: server `grep -c "href=\"/services\""` → 0, `grep -c button_entry` → 17 ✅. HTTPS `/`, `/en/`, `/oferta` 200 ✅.

### Co user musi zrobić w CP

Dla **istniejących stron** używających `service_section`:
- CP → edycja strony → klik na sekcję Service → pole "Strona docelowa buttona" → wybrać stronę (PL: "Oferta", EN: "Services" jeśli istnieje)
- Po zapisie URL automatycznie dopasuje się per locale

Dla **nowych sekcji** picker jest natychmiast dostępny — żadnego hardcodingu.

---

## 2026-06-21 (UPDATE-statamic-6.22.0 + drobne patches + UX nav icon)

### Zmieniono

- **Statamic 6.21.0 → 6.22.0** (minor release wydany 2026-06-19). Krytyczne fixy dla naszego projektu:
  - **Fix nested Bard data loss when editing a set** — używamy Bard sets w `service/show.antlers.html` (quote_section, list_section, image_section, video_section, etc.) → ten fix usuwa ryzyko utraty danych przy edycji nested setów w CP
  - **Retain current tab hash in URL when switching localization** — wielojęzyczna edycja w CP zachowuje pozycję karty
  - **Translate untranslated CP page titles** — lepsza i18n CP
  - **Render asset editor video player with dimensions**
  - **Fix mobile nav button failing**
  - **Don't show selection checkboxes when bulk actions are disabled** (UX)
  - **Fix inconsistent dark mode text colours**
- **Dependency patches:** `phpunit/phpunit` 12.5.28 → 12.5.30, `phpunit/php-code-coverage` 12.5.6 → 12.5.7, `guzzlehttp/uri-template` v1.0.6 → v1.0.7, `ramsey/uuid` 4.9.2 → 4.9.3 (security/stability patches)
- **UX nav icon fix** — `AppServiceProvider.php` ikona `'translate'` (broken, log warning `Icon [translate] not found`) → `'dictionary-language-book'` dla nav item "Tłumaczenia UI" w sekcji Tools. Statamic CP nie ma ikony `translate` — semantycznie najbliższa to `dictionary-language-book` (słownik tłumaczeń).

### Decyzje techniczne

- **`composer update statamic/cms phpunit/phpunit phpunit/php-code-coverage guzzlehttp/uri-template ramsey/uuid --with-dependencies`** — wszystkie 5 paczek + transitive w jednej operacji
- **Patches auto-reaplikowane** przez `cweagans/composer-patches ^2.0` plugin — `patches-relock` / `patches-repatch` NIE były potrzebne (patches.lock.json hash bez zmian = wszystkie HOTFIXy (HOTFIX-10/11/16/17/18 + untranslated-stale) są aktualne dla nowych wersji vendora)
- **Major upgrades odrzucone** (brick/math, hamcrest, inertia, intervention/image, league/glide, openspout, rebing/graphql) — wymagają oddzielnej analizy, ryzyko breaking changes
- **Wzorzec ikon Statamic CP** — nie wszystkie nazwy intuicyjne istnieją (`translate`, `alert`, `btn-danger` jako klasa CSS — wszystkie BROKEN). Zawsze sprawdzać w `vendor/statamic/cms/resources/svg/icons/` lub `public/vendor/statamic/cp/build/assets/app-*.css` (klasy CSS).

### Wdrożono na dev.skalisty.pl

- Pre-deploy backup: `~/skalisty_2026_backups/before-statamic-6.22.0-2026-06-21/` (3 lock files, 488 KB)
- Rsync ~38 MB (vendor/statamic/cms 20 MB + vendor/{phpunit,guzzlehttp,ramsey} 1 MB + public/vendor/statamic/ 19 MB CP assets + 3 lock files)
- Rsync 1 plik AppServiceProvider.php (UX nav icon)
- Post-deploy: config:clear + cache:clear + view:clear + stache:refresh + test (2 passed) — OK
- Walidacja: server `composer.lock` v6.22.0 ✅, patch HOTFIX-18 zachowany ✅, HTTPS 5/5 200 (`/`, `/en/`, `/oferta`, `/cp/auth/login`, `/cp/seo-errors-manager`) ✅

---

## 2026-06-21 (Sync online → offline + backup lokalny)

### Zsynchronizowano (pull `content/` z `--delete` + `public/assets/` bez `--delete`)

**2 nowe usługi services (PL + 11 locale = 24 plików):**
- `groty-jaskinie-skalne` — Groty / Jaskinie skalne
- `rzezby-ze-sztucznego-kamienia` — Rzeźby ze sztucznego kamienia

**Nowy term taxonomy `gallery_tags`:**
- `strona-glowna` (Strona główna) — drugi tag obok istniejącego `oferta`

**Edycja nav PL Oferta mega menu:**
- service_entries: z 2 (sztuczne-skaly + sztuczna-rafa-koralowa) → **5 entries** (dodanie nowych usług + sztuczne-drzewa)

**Nowe assets w `public/assets/galeria/`:**
- `afrykarium/` — 2 nowe webp (afrykarium-9, arykarium-7) + .meta
- `tallinn-zoo/` — nowa galeria (tallinn-zoo-grota.webp + .meta)
- `termy-tarnowskie/` — nowa galeria (termy-tarnowskie-grota-lwa.webp + .meta)

### Stache rebuild

`php artisan cache:clear` + `statamic:stache:clear` + `statamic:stache:warm` — OK (zgodnie z lekcją 2026-06-21: po pull z nowymi taxonomy terms zawsze pełny rebuild, nie incremental refresh).

### Walidacja

- Nav PL Oferta: services_entries count = 5 ✅
- Gallery_tags terms: `oferta`, `strona-glowna` ✅
- HTTP `/` 200 (poprzednio zweryfikowane przy poprzednich syncach)

### Backup lokalny

- **`backup-projekt/skalisty-orion-backup-9.tar.gz`** (373 MB, 27 802 plików)
- Wykluczenia: `node_modules`, `.git`, `storage/framework/{cache,sessions,views}`, `storage/logs`
- Poprzedni backup: backup-8 (2026-06-17, 354 MB) — wzrost +19 MB (nowy content + assets + addon code SEO Errors Manager)

---

## 2026-06-21 (FEATURE-seo-errors-manager — closed/accepted + deployed)

### Dodano

- **CP panel "Błędy SEO (404)"** w sekcji Tools z pełną funkcjonalnością UX:
  - Header z ikoną wykrzyknika + licznikiem wpisów (kontekstowy: "X wpisów" lub "Wyświetlono X z Y" przy filtrze)
  - Toolbar w stylowej karcie (border + rounded-lg + shadow): search input z debounce 500ms, locale dropdown, per-page selector (10/20/50/100/500), bulk delete + delete-all + auto-prune
  - Search filter z aktywnymi chipami pod toolbar + link "Wyczyść wszystkie"
  - Tabela w drugiej karcie: header szare uppercase, hover na wierszach, URL jako klikalny link nowa karta, badge locale
  - Paginator footer z licznikiem `1–N z Total wpisów` (zawsze widoczny) + numerowane buttony stron (window currentPage ± 2 + first/last + "…") + ‹ Poprzednia / Następna ›
  - Empty state z ikoną ✓ + kontekstowy tekst ("Brak błędów" lub "Brak wyników dla filtrów + link Wyczyść")
- **3 akcje delete z różnymi poziomami zabezpieczeń:**
  - Single delete (per row) — confirm + AJAX DELETE
  - Bulk delete (zaznaczone) — confirm z liczbą + AJAX POST
  - **Delete-all (z filtra) — 2-step potwierdzenie:** `confirm()` z ostrzeżeniem ⚠ + `prompt()` z type-to-confirm "USUN" + backend guard `confirm: USUN` w body
- **Auto-prune (Laravel Scheduler bez crona na poziomie command):**
  - Dropdown w toolbar: Wyłączone / 3 / 7 / 15 / 20 / 30 dni
  - Setting persisted w `storage/app/seo-errors-settings.json`
  - `routes/console.php` Schedule::call daily 03:00 → odczyt JSON → wywołanie `seo:errors:prune --older-than=Xd --confirm`
  - `withoutOverlapping()` + `onOneServer()` defensywnie
  - Po uruchomieniu zapisuje `last_pruned_at` + `last_prune_count` → label w UI "Ostatnio: X temu"
- **CLI command `seo:errors:prune`** z 6 opcjami (`--locale`, `--older-than 3d/7d/...`, `--hits-lt`, `--all`, `--confirm`, `--dry-run`) + guard "musisz podać co najmniej jeden filtr lub --all"
- **5 routes CP** w prefix `seo-errors-manager`: index (GET), destroy (DELETE), bulk-delete (POST), delete-all (POST), settings (POST)
- **Nav item** "Błędy SEO (404)" z ikoną `alert-warning-exclamation-mark` w sekcji Tools (po `Translator API`)

### Decyzje techniczne

- **Bez modyfikacji vendora `statamic/seo-pro`** — wykorzystanie publicznego API `Statamic\SeoPro\Facades\Error::*`. `composer update statamic/seo-pro` ma działać normalnie po wdrożeniu.
- **`<script>` w Blade view dla Statamic CP wymaga `@section('scripts')`, NIE `@section('content')`** — Statamic CP używa `<div id="statamic">.innerHTML` jako Vue template (NonInertiaPage), `<script>` w content jest strippowany przez Vue compiler (`[Vue warn]: Tags with side effect (<script> and <style>) are ignored`). Lokalizacja `@section('scripts')` jest renderowana przez `@yield('scripts')` POZA Vue scope.
- **Document-level event delegation** — wszystkie listenery na `document` z `e.target.matches/closest/classList` zamiast cache referencji DOM. Vue mount nadpisuje DOM elementy w `<div id="statamic">`, więc bezpośrednie referencje stają się detached. Delegation na `document` przeżywa re-render.
- **Multisite safety:** klucz route'u to `site::id` (nie sam `id`) — SEO Pro `ErrorsStore::getItemKey()` w multisite dopuszcza ten sam `id` w różnych site. Bez `site::id` delete mógł skasować błąd z niewłaściwego site (odchylenie Codexa od briefu, słuszne).
- **Klasy CSS Statamic CP** — `btn-danger`, `btn-sm`, `btn-primary` **NIE istnieją** w Statamic CP CSS (mimo że `.btn` jest). Buttony muszą używać pełnych Tailwind utility classes (`relative inline-flex items-center justify-center px-X h-Y rounded-lg bg-*-600 hover:bg-*-700 text-white border border-*-700 shadow-ui-md cursor-pointer`). Statamic używa `gray` (z A), nie `grey`. Lekcja z CP-buttons-tailwind-fix (2026-06-07) i tej iteracji.
- **Auto-prune via Laravel Scheduler** — wybrane > on-page-view trigger > natywne `statamic:seo-pro:purge-errors` (ograniczone do `purge_after_days` config bez filtrów). Scheduler daje dynamiczną konfigurację przez UI (zmiana dropdown → setting → następne uruchomienie używa nowej wartości, bez deploy / cache:clear).
- **Default values** — auto_prune_days: 0 (wyłączone domyślnie, opt-in), per_page: 20, sort: last_hit_at desc.

### Analiza gotowych rozwiązań

| Rozwiązanie | Werdykt |
|---|---|
| `Action::register()` Statamic | Odrzucone — SEO Pro Errors używa Inertia/Vue, nie standardowych Resource Listings |
| JS injection do natywnego widoku | Odrzucone — kruche, ryzyko regresji |
| Composer-patches patch | Odrzucone — psuje plugin |
| `statamic:seo-pro:purge-errors` natywne | Częściowo wykorzystane (jako jedno z 3 rozwiązań purge — odrzucone bo tylko config-based bez filtrów). Wybrane custom `seo:errors:prune` z dynamicznym setting + Laravel Scheduler |
| **Custom CP panel paralelny** + **Artisan CLI** + **Laravel Scheduler** | **Wybrane** — pełna kontrola, zero modyfikacji vendora |

### Wdrożono na dev.skalisty.pl

- Pre-deploy backup: `~/skalisty_2026_backups/before-seo-errors-manager-2026-06-21/` (3 pliki, 12 KB).
- Rsync 6 plików (50 KB): `SeoErrorsController.php` (~330 linii), `SeoErrorsPrune.php` (~135 linii), `seo_errors/index.blade.php` (~440 linii), `routes/cp.php` (+5 routes), `routes/console.php` (+33 linie Schedule), `AppServiceProvider.php` (+5 linii nav).
- Post-deploy: config/cache/view clear + stache:refresh + test (2 passed) — OK.
- Walidacja produkcji: route:list pokazuje 5 routes, schedule:list pokazuje `seo-errors-auto-prune` daily 03:00, `php artisan list seo:errors` pokazuje `seo:errors:prune`. HTTP `/cp/seo-errors-manager` 302 → `/cp/auth/login` ✅.

### Konfiguracja crona (do dodania przez usera w panelu dhosting)

```
* * * * * /usr/local/bin/php84 /home/klient.dhosting.pl/skalisty/skalisty_2026/artisan schedule:run >> /dev/null 2>&1
```

Ten jeden wpis obsługuje WSZYSTKIE Laravel scheduled tasks (włącznie z przyszłymi). Bez tego auto-prune nie wykona się o 03:00 (panel CP nadal pełni funkcję, ręczne delete działa).

---

## 2026-06-21 (aktywacja briefu FEATURE-seo-errors-manager)

### Aktywowano

- **FEATURE-seo-errors-manager** (`BRIEF_CODEX.md` state_version 2026-06-21-2200, status active) — brief dla Codex: paralelny CP panel "SEO Errors" w sekcji Tools do zarządzania błędami 404 (`statamic/seo-pro` v7.11.0). 3 endpointy (`GET index`, `DELETE destroy`, `POST bulk-delete`), Blade view z checkboxami + filter locale + sort, link do natywnego "Create Redirect", plus artisan command `seo:errors:prune` (filtry: locale / older-than / hits-lt / all + dry-run + confirm).
- **Zakres:** 3 nowe pliki (`SeoErrorsController.php` ~100 linii, `SeoErrorsPrune.php` ~90 linii, `seo_errors/index.blade.php` ~150 linii) + 2 edycje (`routes/cp.php` ~6 linii, `AppServiceProvider.php` ~4 linie nav item).

### Decyzje techniczne

- **Bez modyfikacji vendora SEO Pro** — wykorzystanie publicznych API `Statamic\SeoPro\Facades\Error::all()`, `Error::find($id)`, `$error->delete()`. `composer update statamic/seo-pro` ma działać po wdrożeniu.
- **Wzorzec ugruntowany w projekcie** — `CollectionRoutesController`, `UiTranslationsController`, `TranslatorApiController` (3 istniejące Tools panels). Nowy panel jest 4-tym wg tego samego wzoru: prefix routes + name group, Blade view `cp/<name>/index.blade.php`, vanilla JS, `Nav::extend()` w `AppServiceProvider`.
- **Permission check minimal** — `User::isSuper() || hasPermission('view seo redirects')` (taka sama jak natywny listing SEO Pro). Dla delete nie tworzymy osobnej permission (uproszczenie, można dodać w przyszłej iteracji).
- **Artisan CLI bonus** — `seo:errors:prune` razem w tym samym briefie (decyzja usera 2026-06-21). Pomocne dla cron jobów i batch cleanup bez UI.
...
|---|---|
| `Action::register()` Statamic | Odrzucone — SEO Pro Errors używa Inertia/Vue, nie standardowych Resource Listings |
| JS injection do natywnego widoku | Odrzucone — kruche, ryzyko regresji przy update |
| Composer-patches patch | Odrzucone — psuje plugin |
| Fork SEO Pro | Odrzucone — overkill |
| **Custom CP panel paralelny** | **Wybrane** — zero zależności od wnętrza vendor, publiczne API SEO Pro |
| **Artisan CLI command** | **Wybrane** jako bonus |

### Uwagi

- Lekcja UI-Translations-Panel (2026-06-02): `routes/cp.php` w Statamic 6 wymaga ręcznej rejestracji przez `Statamic::pushCpRoutes()` w `AppServiceProvider::boot()` — sprawdzić czy jest już zarejestrowane (powinno być z poprzednich paneli).
- Atomowa synchronizacja BRIEF + STATUS + MEMORY (`state_version: 2026-06-21-2200`, `active_task_id: FEATURE-seo-errors-manager`).

---

## 2026-06-21 (HOTFIX-gallery-tags-fieldtype + deploy)

### Naprawiono

- `resources/blueprints/collections/galleries/gallery.yaml` — pole `gallery_tags` zmiana `type: taxonomy` → **`type: terms`**.
- Statamic ma fieldtype **`terms`** (klasa `Statamic\Fieldtypes\Terms`) do wyboru/tworzenia termów taxonomy w polu blueprintu. Fieldtype `taxonomies` (PL `Taxonomies.php`) służy do wyboru samych taxonomy (meta-level), nie termów. **Nie istnieje fieldtype `taxonomy`** — błędna nazwa generowała `FieldtypeNotFoundException` na każdym requestcie `/cp/taxonomies` (TaxonomiesController iteruje blueprinty i próbuje zarejestrować fieldtype).
- Deploy 1 plik (659 B). Walidacja: `/cp/taxonomies` 302 (redirect anon → login) zamiast 500.

### Lekcja

W Statamic Fields API nazwy fieldtype zwykle = lowercased klasa, ale dla taxonomy/terms relationship używa się `terms` (single → wybór termów), nie `taxonomy` (taxonomy field nie istnieje). Doc: https://statamic.dev/fieldtypes/terms

---

## 2026-06-21 (FEATURE-gallery-tags-taxonomy + deploy)

### Dodano

- **Nowa taxonomy `gallery_tags`** (natywny mechanizm Statamic) dla segregacji wpisów kolekcji galleries po przypisaniu do sekcji.
- **Pliki:**
  - `content/taxonomies/gallery_tags.yaml` — definicja taxonomy (title: 'Gallery Tags')
  - `resources/blueprints/taxonomies/gallery_tags/gallery_tag.yaml` — blueprint termu (title + slug, wzór jak `categories`)
  - `content/collections/galleries.yaml` — dodane `taxonomies: [gallery_tags]`
  - `resources/blueprints/collections/galleries/gallery.yaml` — nowe pole `gallery_tags` w sidebar (`type: taxonomy`, `create: true` — inline tagging, `mode: select`, `localizable: false`, brak `max_items` = wiele tagów per galerię)

### Decyzje techniczne (per ustalenia z userem)

- **Nazwa:** `gallery_tags` (generyczna, łatwa do późniejszego ponownego użycia w innych kontekstach niż gallery sections)
- **Frontowe strony tagów (route):** NIE — taxonomy służy wyłącznie do segregacji w CP, brak `route:` w `gallery_tags.yaml`, nie ma `/galerie/tag/{slug}` na froncie
- **Multilingual:** NIE — `localizable: false` na polu, brak `sites:` w taxonomy. Tagi są wewnętrzną metadanymi, niezależnymi od locale.
- **Demo Orion `categories` taxonomy:** zostawiona (do późniejszej czystki przez usera). Nie wykorzystywana, nie podpięta do żadnej kolekcji.
- **Mechanizm tagging:**
  - Centralne zarządzanie: CP → Taxonomies → Gallery Tags → Add Term (rename, delete, slug edit)
  - Inline tagging: w polu na wpisie galleries można wpisać nowy tag (`create: true`) — taxonomy term zostanie utworzony automatycznie

### Analiza gotowych rozwiązań

- **Native first** — Statamic Taxonomies wbudowane, zero customu, CP UI, filtr w listingu kolekcji
- Odrzucone: `tags` text field z `taggable` (brak central management → literówki), `entries` field do innej kolekcji (overengineering), addon community (brak potrzeby — natywne wystarcza)

### Wdrożono na dev.skalisty.pl

- Rsync 4 plików (1.956 B sent). Post-deploy: view:clear + cache:clear + statamic:stache:refresh — OK.
- Walidacja lokalnie: `Taxonomy::find('gallery_tags')` → OK, `Collection::find('galleries')->taxonomies()` → 1 (gallery_tags), `Blueprint::find('collections.galleries.gallery')->field('gallery_tags')` → OK (type=taxonomy) ✅
- Walidacja serwera: `Taxonomy::find('gallery_tags')` → OK ✅

### Co user zobaczy w CP

- `/cp/taxonomies/gallery_tags` — nowa pozycja w nawigacji CP
- `/cp/collections/galleries/entries/{id}/edit` — sidebar ma nowe pole "Tagi (sekcje)" — dropdown z istniejącymi tagami + możliwość wpisania nowego (inline create)
- `/cp/collections/galleries` (listing) — filtrowanie po tagach dostępne w UI listy

---

## 2026-06-21 (HOTFIX-services-icon-svg-store-as + deploy)

### Naprawiono

- Przywrócono `store_as: svg_data` w polu `icon_svg` blueprintu `resources/blueprints/collections/services/service.yaml` — regresja po edycji blueprintu w CP przez usera (sync z 2026-06-21). 1 linia YAML.
- Spójność z innymi polami Iconify w projekcie: `resources/fieldsets/icon_box_with_text_section.yaml` i `resources/fieldsets/logos_slider_with_icons.yaml` (oba miały `store_as: svg_data` jawnie). Zgodne z wytyczną FEATURE-iconify-install (2026-06-17): "w polach blueprintu używać `store_as: svg_data`" (eksplicit > implicit, mimo że config ma `default_store_as: svg_data`).

### Decyzje techniczne

- **Eksplicit > implicit** — pole ma `store_as: svg_data` jawnie nawet gdy config global ma `default_store_as: svg_data`. Powód: zmiana defaultu w przyszłości nie zaskoczy zachowaniem blueprintu, blueprint dokumentuje sam siebie.
- **Pozostawiono istniejące wpisy w obecnym formacie** (np. `content/collections/services/pl/sztuczne-skaly.md` z `icon_svg: {name: 'game-icons:rock', attributes: {...}}` zamiast inline SVG). Re-zapis przez CP / batch artisan jako osobne zadanie — user wykona ręcznie 2 wpisy PL (`sztuczne-skaly`, `sztuczna-rafa-koralowa`).

### Wdrożono na dev.skalisty.pl

- Rsync 1 plik (947 B sent). Post-deploy: view:clear + cache:clear + statamic:stache:refresh — OK.
- Walidacja: serwer ma `store_as: svg_data` w `icon_svg` ✅.

---

## 2026-06-21 (FEATURE-services-pagebuilder-defaults + deploy)

### Dodano

- **5 domyślnych setów w page_builder dla blueprintu `service`**: `overview_section`, `image_with_text_section`, `how_it_works_section`, `skalisty_gallery_section`, `confidence_section`. Każdy nowy wpis services w CP dostaje te 5 setów automatycznie (jak `details` w blueprintcie projects od 2026-06-19).
- Plik zmodyfikowany: `resources/blueprints/collections/services/service.yaml` — pole `page_builder` import zmienione z prostego `import: all_page_builder` na import + `config.page_builder.default` z 5 setami (~22 linie YAML).

### Decyzje techniczne

- **Override config przy `import:` musi być mapą per handle pola** — vendor Statamic `Fields::getImportedFields` (linia 286-291) merge'uje config z `$overrides[$handle] ?? []`. Czyli format: `config: <field_handle>: <override_config>`. NIE działa prosty `config: default: [...]` (próbowane pierwszy raz, blueprint widział default jako null).
- **Wybór override per blueprint vs zmiana `all_page_builder.yaml`** — zachowano lokalność (zmiana tylko w `service.yaml`). Dodanie `default` w `all_page_builder.yaml` wymuszałoby 5 setów na każdy nowy wpis w pages/projects/blogs/teams/itd. — niepożądane.
- **Zachowanie mechanizmu importu** — kolejne sety dodane w przyszłości do `all_page_builder.yaml` nadal pojawiają się jako *opcje* w page_builder dla services (override dotyczy tylko `default` initial state).

### Diagnoza poprzedniego problemu (NIE wprowadzono zmian)

Wcześniej user próbował dodać confidence_section jako sekcję do blueprintu service przez CP Blueprint Editor. Statamic 6.21 rzucał `DuplicateFieldException: Duplicate field [description] on blueprint [service]` (i wcześniej `[image]`) bo confidence_section ma top-level pola `description` + `image` które kolidują z istniejącymi top-level polami service blueprint. **To podejście było złe** — CP Blueprint Editor nie ma UI dla `default:` setów replicatora. User został poinformowany, sam usunie nieudane próby przez CP. Aktualna zmiana (`config.page_builder.default`) realizuje intencję poprzez właściwy mechanizm Statamic.

### Wdrożono na dev.skalisty.pl

- Pre-deploy backup: `~/skalisty_2026_backups/before-services-page-builder-defaults-2026-06-21/` (1 plik, 20 KB)
- Rsync 1 plik (1.492 B sent)
- Post-deploy: `view:clear` + `cache:clear` + `statamic:stache:refresh` — OK
- Walidacja: `Blueprint::find("collections.services.service")->field("page_builder")->config()["default"]` → 5 setów ✅

---

## 2026-06-21 (Sync — pull serwer → lokalnie po edycjach usera w CP)

### Zsynchronizowano (pull `content/` z `--delete` + `assets/` bez `--delete`)

- **12 nowych usług demo Orion** (`bridge-construction`, `building-construction`, `building-renovation`, `flooring-roofing-solutions`, `infrastructure-development`, `interior-fit-out-finishing`, `landscape-construction`, `pre-construction-planning`, `project-management`, `road-highway-construction`, `steel-structure-work`, `architectural-design`) — w PL i EN. **PL ma 12 demo + 2 realne (`sztuczne-skaly`, `sztuczna-rafa-koralowa`) = 13. EN ma 12 demo. 10 pozostałych locale ma tylko `sztuczne-skaly` (lokalizacja PL)**.
- **24 nowe lokalizacje galerii** (`galeria-sztuczne-skaly-wstep.md` + `oferta-sztuczne-skaly-galeria.md` × 12 locale).
- **Nav PL — Oferta mega menu**: user wymienił 7 demo Orion service entries → **2 realne** (`sztuczne-skaly` + `sztuczna-rafa-koralowa`).
- **Strona PL Oferta** (`content/collections/pages/pl/oferta.md`): dodano `section_title: 'Lista Usług'` w `services_grid_section`.

### Zmiany blueprintu (poza scope edycji usera w CP, do uwagi)

- `resources/blueprints/collections/services/service.yaml`:
  - Dodano `localizable: false` na polach `subtitle`, `icon`, `icon_svg`, `image`, `description`, `button_label` — pola dziedziczą z PL przez origin chain.
  - **⚠ Usunięto `store_as: svg_data` z pola `icon_svg`** — to **regresja** względem decyzji `FEATURE-iconify-install` (2026-06-17): bez `svg_data` Iconify ładuje ikony przez API zamiast embed offline. Wymaga decyzji usera: przywrócić czy świadomie zmienić podejście?

### Status na froncie lokalnym

- `http://127.0.0.1:8001/oferta` → 200; `services_grid_section` renderuje 3 published services: `architectural-design`, `sztuczne-skaly`, `sztuczna-rafa-koralowa`. 12 demo Orion PL services ma `published: false` — niewidoczne na froncie.
- `http://127.0.0.1:8001/` → 200; mega menu PL Oferta pokazuje 2 entries: `sztuczne-skaly` + `sztuczna-rafa-koralowa`.

### Otwarte pytania do usera

1. **`store_as: svg_data` w `icon_svg`** — przywrócić? (regresja względem FEATURE-iconify-install)
2. **12 demo Orion PL services `published: false`** — zachować jako template/inspiracja, czy usunąć?
3. **`architectural-design`** — istnieje tylko w EN (brak PL); renderuje się też w PL /oferta. Usunąć z EN, czy zostawić?

### Stan na koniec sync

- Stache:refresh: OK
- HTTP / + /oferta: 200
- Lokalnie 13 services PL (3 published na froncie) + 12 services EN
- Lokalny content/ = serwer dev.skalisty.pl

---

## 2026-06-21 (Deploy FEATURE-mega-menu-globals-i18n na dev.skalisty.pl)

### Wdrożono

- **23 pliki** z `FEATURE-mega-menu-globals-i18n` na `dev.skalisty.pl`: blueprint `mega_menu` + 12 plików per locale (PL ręcznie + 11 DeepL), edycje `TranslateGlobalSet.php`, 4 headery `header-{1,2,3,4}.antlers.html`, blueprint `footer.yaml` default, `content/globals/en/footer.yaml`, 2 nav trees (PL/EN). Łącznie 158 KB transferowanych.
- Pre-deploy backup serwera: `~/skalisty_2026_backups/before-mega-menu-globals-i18n-2026-06-21/` (9 plików, 164 KB).
- Post-deploy: `config:clear` + `cache:clear` + `view:clear` + `statamic:stache:refresh` + `test` (2 passed) — wszystko OK.

### Walidacja produkcji

- `https://dev.skalisty.pl/` → 200, mega menu po PL (`Przeglądaj Realizacje` / `Sprawdź Naszą Ofertę` / `Wszelkie prawa zastrzeżone`)
- `https://dev.skalisty.pl/en/` → 301 → `/en` 200, mega menu po EN (`Browse Our Projects` / `Check Out Our Offer` / `All rights reserved by Skalisty Group`)
- `/cs/ /da/ /es/` → 200 (aktywne locale)
- `/de/ /fr/ /sv/ /no/ /it/ /nl/ /lv/` → 301 do `/` (znany fallback)
- Demo Orion `All rights reserved to Orion` w HTML /en/ = 0 trafień ✅

### Uwagi

- Auto-mode Claude Code (v2.1.15x) zablokował początkowy SSH write na production — user wyłączył `/auto`, deploy ukończony.
- Insync wrapper SSH (`/home/pestycyd/Insync/.../skalisty-ssh`) — działający dla SSH z argumentami; nie `/usr/bin/skalisty-ssh` (otwiera tylko interaktywną sesję).
- Wpis pełnej dokumentacji w `server_deploy/DEPLOYMENT.md` (sekcja 2026-06-21).

---

## 2026-06-21 (FEATURE-mega-menu-globals-i18n — closed/accepted)

### Dodano

- **Nowy global Statamic `mega_menu`** z 3 polami `localizable: true` (`copyright_text`, `projects_button_text`, `services_button_text`), defaultsy PL ("© Wszelkie prawa zastrzeżone przez Skalisty Group sp. z o.o.", "Przeglądaj Realizacje", "Sprawdź Naszą Ofertę"). 12 plików per locale (PL ręcznie + 11 przez DeepL). Origin chain PL → 11 locale.
- **Wpis `mega_menu` w `TRANSLATABLE_FIELDS`** w `app/Console/Commands/TranslateGlobalSet.php` (analogicznie do `touch_with_us`).
- **Wrappery `{{ mega_menu }}...{{ /mega_menu }}`** w 4 widokach `header-{1,2,3,4}.antlers.html` (po 4 wrappery per plik, 16 nowych wrapperów łącznie).

### Zmieniono

- **`header-{1,2,3,4}.antlers.html`** — refactor na **override z fallbackiem**: pole `button_text` / `copyright_text` z nav item ma priorytet, w razie pustki widok bierze wartość z globala (wrapper `{{ mega_menu }}{{ if projects_button_text }}...{{ /if }}{{ /mega_menu }}` per mega menu: project-list → `projects_button_text`, service-list → `services_button_text`). +36 linii per plik (4 fallbacki: 2 copyright + 1 projects_button + 1 services_button).
- **`content/globals/en/footer.yaml`** — `copyright_text: '© All rights reserved to Orion Constructions'` → `'© All rights reserved by Skalisty Group sp. z o.o.'` (poza ścisłym scope navigation, ale konieczne dla kryterium "brak demo Orion w HTML /en/").
- **`resources/blueprints/globals/footer.yaml`** — default `copyright_text`: "Orion Constructions" → "Wszelkie prawa zastrzeżone przez Skalisty Group sp. z o.o." (cosmetic — wszystkie 10 lokale dziedziczą z PL przez origin chain footer).

### Naprawiono

- **Demo Orion w nav trees PL/EN** — usunięto wpisy `button_text:` i `copyright_text:` z bloków project-list i service-list w `content/trees/navigation/pl/main.yaml` (4 linie) i `content/trees/navigation/en/main.yaml` (4 linie). PL service-list miał `'View Services'` + Orion copyright (slip Orion w PL tree). EN miał demo Orion w obu mega menu. 10 pozostałych locale miało już `tree: []`.

### Decyzje techniczne

- **Architektura override z fallbackiem** (decyzja użytkownika 2026-06-21) zaakceptowana w pełni przez audyt — daje elastyczność (różne CTA per nav item nadal możliwe) + auto-tłumaczenie dla 12 locale bez ręcznej edycji per locale.
- **Wzorzec rozszerzenia `TRANSLATABLE_FIELDS`** w `TranslateGlobalSet.php` dla każdego nowego globala — bez tego DeepL nie zostaje wywołany (komenda skopiuje source as-is z ostrzeżeniem).
- **Mega menu na /cs/ /da/ /es/** nie renderuje się fizycznie bo nav trees mają `tree: []` (istniejący stan z sesji 2026-06-07) — **wartość dodana briefu:** gdy user doda nav items dla nowego locale w CP, mega menu automatycznie podciągnie tłumaczenie z globala. Zero ręcznej pracy per locale w przyszłości.

### Analiza gotowych rozwiązań

Pominięta per AGENTS.md 9.7 — zadanie wykorzystywało istniejący wzorzec (`touch_with_us` global + `globals:translate` komenda).

### Uwagi

- ES: DeepL użył tego samego zwrotu `Echa un vistazo a` dla obu CTA (`Echa un vistazo a nuestros proyectos` / `Echa un vistazo a nuestra oferta`). Akceptowalne; user może edytować w CP > Globals > Mega Menu jeśli chce zróżnicować.
- "Orion Construction" nadal w HTML /en/ — 4 trafienia w testimonialach (`reviews_section`) i cookie popup (`newsletter_cookies`). To **demo content kolekcji, poza scope** tego briefu. Kandydat na osobny ticket "cleanup demo Orion content".
- `/de/ /fr/ /sv/ /no/ /it/ /nl/ /lv/` zwracają 302 do `/` (fallback locale routing z bootstrap/app.php sesja 2026-05-31) — Codex słusznie zauważył niespójność z kryterium akceptacji briefu. Nie regresja.
- Brief zarchiwizowany w `briefs/archive/2026-06-21-feature-mega-menu-globals-i18n.md`.

---

## 2026-06-21 (aktywacja briefu)

### Aktywowano

- **FEATURE-mega-menu-globals-i18n** (`BRIEF_CODEX.md` state_version 2026-06-21-1530, status active) — brief dla Codex: nowy global `mega_menu` z 3 polami `localizable: true` (`copyright_text`, `projects_button_text`, `services_button_text`), refactor 4 widoków `header-{1,2,3,4}.antlers.html` na fallback override-z-globalem, rozszerzenie `TRANSLATABLE_FIELDS` w `app/Console/Commands/TranslateGlobalSet.php`, czystka 12 nav trees z demo Orion (`View Services`, `All rights reserved to Orion Constructions`), auto-tłumaczenie 11 locale przez `globals:translate mega_menu`. Atomowa synchronizacja BRIEF + STATUS + MEMORY.

### Decyzje techniczne

- **Architektura override z fallbackiem** (decyzja użytkownika 2026-06-21) — pola `button_text` i `copyright_text` w blueprincie navigation zachowane jako **opcjonalny override per nav item**. Jeśli puste → fallback `{{ if button_text }}...{{ else }}{{ mega_menu }}{{ projects_button_text }}{{ /mega_menu }}{{ /if }}` do globala. Daje elastyczność (różne CTA per nav item są możliwe) + auto-tłumaczenie dla 12 locale bez ręcznej edycji per locale.
- **Wzorzec wieloglobala** — DeepL przez `globals:translate` (komenda projektu od 2026-06-06) wymaga rozszerzenia stałej `TRANSLATABLE_FIELDS` w `TranslateGlobalSet.php` dla każdego nowego globala — bez tego komenda skopiuje source as-is bez wywołania DeepL (`warn: "No translatable field definitions"`). Wpis dla `mega_menu` dodaje się analogicznie do istniejącego `touch_with_us`.
- **Czystka navigation trees** — wszystkie wystąpienia `button_text:` i `copyright_text:` w 12 plikach `content/trees/navigation/{locale}/main.yaml` usuwane. PL tracąc nic (te same wartości w globalu PL), EN i 10 locale tracą demo Orion (`View Projects`, `View Services`, `All rights reserved to Orion Constructions`).

### Analiza gotowych rozwiązań

Pominięta per AGENTS.md 9.7 — zadanie wykorzystuje istniejący wzorzec (`touch_with_us` global + `globals:translate` komenda), nie jest nową większą funkcjonalnością.

### Uwagi

- Magic Translator **nie obsługuje** navigation trees (komenda `statamic:magic-translator:translate` operuje wyłącznie na entries kolekcji per `vendor/el-schneider/statamic-magic-translator/src/Commands/TranslateCommand.php`). Globalsy + custom `globals:translate` to jedyna prosta ścieżka i18n dla nav chrome.
- Wrappery `{{ mega_menu }}...{{ /mega_menu }}` w partialach są obowiązkowe (HOTFIX-14 lesson z 2026-06-02 — Statamic 6 nie kaskaduje globalsów do partials automatycznie).
- Override-fallback w Antlers: używać `{{ if X }}...{{ else }}...{{ /if }}` zamiast null coalescing `??` (Antlers ma ograniczenia w warunkach z `??`).
...
- **`services_grid_section.section_button.url`** — dodane `localizable: true` + `translatable: false`. Pole jest teraz per-locale (redaktor może wpisać `/oferta` PL i `/services` EN ręcznie), ale Magic Translator nie próbuje automatycznie tłumaczyć URL przez DeepL. Tłumaczenie URL semantycznie nie ma sensu — to przekład hasła, nie ścieżki.
- Zaktualizowane `instructions` pola — wyjaśnia że pole jest per-locale + Magic Translator pomija.

### Magic Translator — status pól `services_grid_section`

Pola **tłumaczone** (Tier 1 — `text`/`textarea`/`wysiwyg_html` + `localizable: true`):
- `section_title`, `tag_title`, `section_button.text`, `card_button_text`

Pola **per-locale ręczne** (Statamic localizable + Magic Translator pomija):
- `section_button.url` (`translatable: false`)

Pola **globalne** (nie translatable, nie localizable):
- `layouts_grid` (select), `collections`, `limit`, `asymmetric_entries`

### Walidacja

- YAML poprawny (`localizable: True`, `translatable: False`)
- Deploy: `rsync` fieldsetu + `stache:refresh` na `dev.skalisty.pl` ✅
